...
This script sets up a webhook listener for Drone's global webhooks. It then sends a notification to a Matrix chat every time a build passes or fails.

## Getting Started

Run the docker container with a config file (notify.toml) mounted to /config/notify.conf. Configure the required parameters.
...
services:
  ...
  notify:
    image: spritsail/drone-notify:2.0
    volumes:
      - path/to/notify.conf:/config/notify.toml
```
